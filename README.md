...
!setconfig level_up_channel           ← канал за level-up съобщения
!setconfig log_channel                ← канал за XP логове
!setconfig stats_channel              ← канал за !top класация
!setconfig admin_log_channel          ← канал за модерация логове + crew одобрения
!setconfig welcome_channel            ← канал за нови членове
!setconfig belly_rush_channel         ← канал за Belly Rush панела
!setconfig belly_rush_roles_channel   ← канал за !want команди
!setconfig reminders_channel          ← канал за напомняния
!setconfig repair_channel             ← канал за repair-ship команди
!setconfig translator_channel         ← канал за двупосочен AI превод
!setconfig bot_status_channel         ← канал за Online/Offline статус
!setconfig bot_info_channel           ← канал за мануала с командите
!setconfig unit_build_channel         ← канал за !hero команди
!setconfig bounty_channel             ← канал за !wanted плакати
!setconfig rules_channel              ← канал с правилата (за welcome)
!setconfig general_channel            ← general chat (за welcome)
!setconfig rookies_role               ← роля за нови членове (авт. се създава)
!setconfig player_role                ← роля след верификация (авт. се създава)
!setconfig mod_role                   ← роля за модератори
!setconfig restricted_channel         ← канал с ограничения за менции
!setconfig protected_users     <id1,id2>  ← защитени потребители (ID-та с ,)
!setconfig bday_channel               ← канал за birthday (опционален)
!setconfig bday_user                  ← user ID за birthday (опционален)
...
!addroleallgm @роля        ← дава роля на всички с ᴹ☠️
...
!setconfig level_up_channel           ← channel for level-up messages
!setconfig log_channel                ← channel for XP logs
!setconfig stats_channel              ← channel for !top leaderboard
!setconfig admin_log_channel          ← channel for mod logs + crew approvals
!setconfig welcome_channel            ← channel for new members
!setconfig belly_rush_channel         ← channel for the Belly Rush panel
!setconfig belly_rush_roles_channel   ← channel for !want commands
!setconfig reminders_channel          ← channel for reminders
!setconfig repair_channel             ← channel for repair-ship commands
!setconfig translator_channel         ← channel for two-way AI translator
!setconfig bot_status_channel         ← channel for Online/Offline status
!setconfig bot_info_channel           ← channel for the bot manual
!setconfig unit_build_channel         ← channel for !hero commands
!setconfig bounty_channel             ← channel for !wanted posters
!setconfig rules_channel              ← rules channel (for welcome message)
!setconfig general_channel            ← general chat (for welcome message)
!setconfig rookies_role               ← role for new members (auto-created)
!setconfig player_role                ← role after verification (auto-created)
!setconfig mod_role                   ← moderator role
!setconfig restricted_channel         ← channel with mention restrictions
!setconfig protected_users     <id1,id2>  ← protected users (IDs separated by ,)
!setconfig bday_channel               ← birthday channel (optional)
!setconfig bday_user                  ← birthday user ID (optional)
...
!leveling-enable    ← activate XP for THIS server
...
!translate-enable    ← activate for THIS server
...
!auto-translate-enable    ← activate for THIS server
...
!addroleallgm @role        ← give role to everyone with ᴹ☠️
...
good morning / dobro утро  ← bot replies with a random good morning GIF
...
> 🔐 **Leveling, Flag Translate and Auto-Translate** are activated with a password from the bot o
