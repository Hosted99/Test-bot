...
client.on("messageCreate", async (msg) => {
    ...
    if (msg.content.startsWith("!")) {
        ...
        // !checkconfig — показва какво е конфигурирано и какво липсва
        if (cmd === "!checkconfig") {
            if (!msg.member.permissions.has("Administrator")) return;
            ...
            // Помощна функция за разделяне на дълги списъци на части по 1024 символа
            const splitFields = (arr, title) => {
                const fields = [];
                let chunk = "";
                for (const line of arr) {
                    if ((chunk + line + "\n").length > 1000) {
                        fields.push({ name: fields.length === 0 ? title : `${title} (продължение)`, value: chunk.trim() });
                        chunk = "";
                    }
                    chunk += line + "\n";
                }
                if (chunk) fields.push({ name: fields.length === 0 ? title : `${title} (продължение)`, value: chunk.trim() });
                return fields;
            };

            const description = missing.length === 0
                ? "🎉 **Всичко е конфигурирано! Ботът е готов за работа.**"
                : `⚠️ **${missing.length} настройки липсват.** Използвай \`!setconfig <key> <value>\` за да ги добавиш.`;
            ...
            if (configured.length > 0) {
                splitFields(configured, `✅ Конфигурирано (${configured.length})`).forEach(f => embed.addFields(f));
            }
            if (missing.length > 0) {
                splitFields(missing, `❌ Липсва (${missing.length})`).forEach(f => embed.addFields(f));
            }
            if (optional.length > 0) {
                splitFields(optional, `⚪ Опционално (${optional.length})`).forEach(f => embed.addFields(f));
            }
