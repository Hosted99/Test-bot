...
client.on("messageCreate", async (msg) => {
    ...
    if (msg.content.startsWith("!")) {
        ...
        if (cmd === "!setconfig") {
            ...
            const value = args[1];
            if (!key || !value) {
                return msg.reply("❌ Формат: `!setconfig <ключ> <стойност>`\n\nНаличните ключове:\n`level_up_channel`, `log_channel`, `stats_channel`, `admin_log_channel`, `welcome_channel`, `belly_rush_channel`, `reminders_channel`, `repair_channel`, `translator_channel`, `bot_status_channel`, `rookies_role`, `player_role`, `restricted_channel`, `protected_users`");
            }
        ...
        if (cmd === "!mania-addguild") {
            ...
            if (!key || !roleId || !channelId) {
                return msg.reply("❌ Usage: `!mania-addguild <key> @role #channel`\nExample: `!mania-addguild g3 @Guild3 #mania-g3`");
            }
            await setConfig(msg.guild.id, `mania_role_${key}`, roleId);
            await setConfig(msg.guild.id, `mania_channel_${key}`, channelId);
            return msg.reply(`✅ Guild **${key.toUpperCase()}** added!\n🎭 Role: <@&${roleId}>\n📣 Channel: <#${channelId}>`);
        }

        // !mania-removeguild <key>
        if (cmd === "!mania-removeguild") {
            ...
            await deleteConfig(msg.guild.id, `mania_channel_${key}`);
            return msg.reply(`✅ Guild **${key.toUpperCase()}** removed.`);
        }

        // !mania-guilds — показва всички настроени гилдии
        if (cmd === "!mania-guilds") {
            if (!msg.member.permissions.has('Administrator')) return;
            const { getAllConfig } = require('./utilities/guildConfig');
            const config = await getAllConfig(msg.guild.id);
            const guilds = Object.keys(config)
                .filter(k => k.startsWith('mania_role_'))
                .map(k => {
                    const key = k.replace('mania_role_', '');
                    const roleId = config[`mania_role_${key}`];
                    const channelId = config[`mania_channel_${key}`];
                    return `**${key.toUpperCase()}** — Role: <@&${roleId}> | Channel: ${channelId ? `<#${channelId}>` : '⚠️ not set'}`;
                });
            if (guilds.length === 0) return msg.reply("⚠️ No mania guilds configured. Use `!mania-addguild`");
            const { EmbedBuilder } = require('discord.js');
            const embed = new EmbedBuilder()
                .setTitle("⚔️ Mania Guilds")
