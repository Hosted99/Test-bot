...
async function handleCommands(msg, pool) {
    ...
    if (cmd === "!setbounty") {
        ...
        try {
            ...
            // ✅ Винаги изпраща в bounty_channel, независимо откъде е написана командата
            const bountyChannel = await getChannel(msg.guild, 'bounty_channel');
            if (bountyChannel) {
                await bountyChannel.send({ embeds: [embed] });
                if (msg.channel.id !== bountyChannel.id) {
                    const notice = await msg.channel.send(`✅ Bounty updated! Check <#${bountyChannel.id}>.`);
