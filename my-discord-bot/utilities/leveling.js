...
const { getChannel, getConfig } = require("./guildConfig"); // ← MULTI-SERVER CONFIG / МУЛТИ-СЪРВЪР КОНФИГ
...
async function saveToDatabase(pool, guildId, userId, data) {
    // ✅ PER-GUILD: save XP, level and guild_name for easy DB navigation / записваме XP, ниво и guild_name
...
module.exports = (client, poolObj) => {
    ...
    async function loadCacheFromDB() {
        try {
            // ✅ PER-GUILD: load all profiles with guild_id / зареждаме всички профили с guild_id
            const res = await pool.query('SELECT guild_id, guild_name, user_id, xp, level, username FROM levels');
            res.rows.forEach(row => {
                const cacheKey = `${row.guild_id}:${row.user_id}`; // уникален ключ per-guild
            ...
            console.log(`[System] Loaded ${res.rowCount} profiles from Neon. / Заредени ${res.rowCount} профила.`);
        } catch (e) { console.error("Cache load error / Грешка при зареждане:", e); }
    }

    loadCacheFromDB();

    client.on('messageCreate', async (message) => {
        // ✅ MULTI-SERVER: works for EVERY server / работи за ВСЕКИ сървър
        if (message.author.bot || !message.guild) return;

        // ✅ Leveling is DISABLED by default — activate with !leveling-enable <password>
        // ✅ Leveling е ИЗКЛЮЧЕН по подразбиране — активира се с !leveling-enable <парола>
        const levelingEnabled = await getConfig(message.guild.id, 'leveling_enabled');

        // Handle enable/disable commands regardless of leveling state
        // Обработваме командите за вкл/изкл независимо от статуса
        if (message.content.toLowerCase().startsWith('!leveling-enable')) {
            ...
            if (!storedPassword) {
                return message.reply('❌ LEVELING_PASSWORD not set in .env!')
            ...
            const msg = await message.reply('✅ **Leveling system activated!** XP is now being tracked.');
        ...
        if (message.content.toLowerCase().startsWith('!leveling-disable')) {
            ...
            const msg = await message.reply('🔒 **Leveling system disabled.**');
            setTimeout(() => { message.delete().catch(() => {}); }, 3000);
            return;
        }

        // If leveling is not enabled — stop here / Ако не е активиран — спираме
        if (levelingEnabled !== 'true') return;

        const userId = message.author.id;
        const guildId = message.guild.id;
        const cacheKey = `${guildId}:${userId}`; // ✅ PER-GUILD: unique cache key / уникален ключ

        let userData = xpCache.get(cacheKey);
        if (!userData) {
            // ✅ PER-GUILD: search by guild_id + user_id / търсим по guild_id + user_id
        ...
        // ✅ Always update guildName from current guild (fixes 'unknown' for old rows) / Винаги обновяваме guildName
        ...
        if (message.content.toLowerCase() === '!top') {
            ...
            // ✅ PER-GUILD: top only for THIS server / топ само за ТОЗИ сървър
            ...
            const embed = new EmbedBuilder().setTitle(v.t).setDescription(v.d).setColor('#FF4500').setThumbnail(message.guild.iconURL({ dynamic: true } || null)).setFooter({ text: '☠️ Vanish in 60s' }).setTimestamp();
        ...
        if (message.content.toLowerCase() === '!sync' && message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            ...
            for (const [key, data] of xpCache.entries()) {
                if (data.needsUpdate) {
                    ...
                    // ✅ Get guild_name from client.guilds.cache — never unknown / Взимаме guild_name
    ...
    cron.schedule('59 23 * * 0', async () => {
        client.guilds.cache.forEach(async (guild) => {
            ...
            try {
                // ✅ PER-GUILD: leaderboard only for THIS server / класация само за ТОЗИ сървър
    ...
    cron.schedule('0 */2 * * *', async () => {
        try {
            ...
            for (const [key, data] of xpCache.entries()) {
                if (data.needsUpdate) {
                    const [gId, uId] = key.split(':'); // ✅ PER-GUILD: split the cache key / разделяме ключа
                    // ✅ Get guild_name from client.guilds.cache — never unknown / Взимаме guild_name
            ...
            client.guilds.cache.forEach(async (guild) => {
                ...
                if (logChannel) {
                    const autoSyncEmbed = new EmbedBuilder().setColor(0x2ecc71).setTitle('♻️ Automatic Sync Executed').setDescription(`Updated **${count}** pirate profiles.`).setTimestamp();
