...
module.exports = (client, poolObj) => {
    ...
    client.on('messageCreate', async (message) => {
        ...
        // ✅ Проверяваме дали leveling е включен за ТОЗИ сървър
        // По подразбиране е включен — спира се с !setconfig leveling_enabled false
        ...
        // ✅ Винаги обновяваме guildName от текущия сървър (оправя 'unknown' за стари редове)
        ...
        if (message.content.toLowerCase() === '!sync' && message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            ...
            for (const [key, data] of xpCache.entries()) {
                if (data.needsUpdate) {
                    ...
                    // ✅ Взимаме guild_name от client.guilds.cache — никога unknown
