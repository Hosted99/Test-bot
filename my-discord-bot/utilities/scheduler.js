...
async function handleManiaList(msg) {
    ...
    try {
        ...
        // ✅ Фиксирани канали (като mania-plan), независимо къде е писана командата:
        //   FORMATION STATUS + Attention! → plan канала (#mania-strategy)
        //   MANDATORY! + линк → notify канала (#general)
        // planChannel вече е каналът, в който е публикуван планът.
        await planChannel.send({ embeds: [statusEmbed] });

        if (missing.length > 0) {
            // Attention! списък → plan канала
            ...
            // MANDATORY! списък + линк към плана → notify канала
            if (guildData.notifyChannelId) {
                const notifyChannel = msg.client.channels.cache.get(guildData.notifyChannelId)
                    || await msg.client.channels.fetch(guildData.notifyChannelId).catch(() => null);
                if (notifyChannel) {
                    await sendLongMessage(
                        notifyChannel,
                        `🚨 **MANDATORY!** Members of **${arg.toUpperCase()}** need to vote:\n`,
                        missing,
                        " ",
                        `\n\n🔗 **Plan:** ${planMsg.url}`
                    );
            ...
            await planChannel.send(`✅ Everyone from **${arg.toUpperCase()}** has voted!`);
        }
