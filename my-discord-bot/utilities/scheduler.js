...
// ─────────────────────────────────────────────
// Помощни: режат дълги списъци от хора, за да не гръмнат лимитите на Discord
// Embed поле = макс 1024 знака; обикновено съобщение = макс 2000 знака
// ─────────────────────────────────────────────
function chunkEmbedFields(items, baseName, joiner = ", ", emptyText = "None") {
    if (items.length === 0) return [{ name: baseName, value: emptyText, inline: false }];
    const fields = [];
    let chunk = "";
    let part = 1;
    for (const item of items) {
        const piece = (chunk ? joiner : "") + item;
        if (chunk.length + piece.length > 1000) { // 1000 за презастраховка под 1024
            fields.push({ name: part === 1 ? baseName : `${baseName} (cont.)`, value: chunk, inline: false });
            chunk = item;
            part++;
        } else {
            chunk += piece;
    ...
    if (chunk) fields.push({ name: part === 1 ? baseName : `${baseName} (cont.)`, value: chunk, inline: false });
    return fields;
}

async function sendLongMessage(channel, prefix, items, joiner = " ", suffix = "") {
    // Разбива списък на няколко съобщения под 2000 знака
    let chunk = "";
    let first = true;
    for (const item of items) {
        const piece = (chunk ? joiner : "") + item;
        if ((prefix.length + chunk.length + piece.length + suffix.length) > 1900) {
            await channel.send(`${first ? prefix : ""}${chunk}`).catch(() => {});
            chunk = item;
            first = false;
        } else {
            chunk += piece;
    ...
    if (chunk) await channel.send(`${first ? prefix : ""}${chunk}${suffix}`).catch(() => {});
}
...
async function handleManiaList(msg) {
    ...
    try {
        ...
        chunkEmbedFields(confirmed, `✅ CONFIRMED (${confirmed.length})`, ", ", "None yet")
            .forEach(f => statusEmbed.addFields(f));
        chunkEmbedFields(declined, `❌ DECLINED (${declined.length})`, ", ", "None")
            .forEach(f => statusEmbed.addFields(f));

        // ✅ Резултатът излиза в ЗАДАДЕНИЯ канал, а не където е писана командата.
        // Приоритет: notify канал → plan канал → текущия (резервен вариант).
        let outputChannel = msg.channel;
        const preferredId = guildData.notifyChannelId || guildData.planChannelId;
        if (preferredId) {
            const ch = msg.client.channels.cache.get(preferredId)
                || await msg.client.channels.fetch(preferredId).catch(() => null);
            if (ch) outputChannel = ch;
        }

        await outputChannel.send({ embeds: [statusEmbed] });

        if (missing.length > 0) {
            await sendLongMessage(
                outputChannel,
                `🔔 **Attention!** These players from **${arg.toUpperCase()}** haven't voted yet:\n`,
                missing,
                " ",
                `\n\n🔗 **Plan:** ${planMsg.url}`
            );
        } else {
            await outputChannel.send(`✅ Everyone from **${arg.toUpperCase()}** has voted!`);
        }

        // Кратко потвърждение там, където е писана командата (ако е друг канал)
        if (outputChannel.id !== msg.channel.id) {
            await msg.channel.send(`✅ Formation status for **${arg.toUpperCase()}** posted in <#${outputChannel.id}>.`)
