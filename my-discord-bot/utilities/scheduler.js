...
// ─────────────────────────────────────────────
// Помощна: взима всички гилдии за сървъра
...
async function handleManiaPlan(msg) {
    ...
    if (guilds.length === 0) {
        return msg.reply("❌ Няма настроени гилдии. Използвай `!mania-addguild <key> @role #plan-channel #notify-channel`");
    }
    ...
    if (targets.length === 0) {
        ...
        return msg.reply(`❌ Гилдия \`${arg}\` не е намерена. Налични: ${keys} или \`all\``);
    }
...
async function handleManiaList(msg) {
    ...
    if (!arg || !guildData) {
        const keys = guilds.map(g => `\`${g.key}\``).join(', ');
        return msg.reply(`❌ Използвай: \`mania-list <key>\`. Налични: ${keys}`);
    }
    ...
    try {
        ...
        // Намираме канала в който е планът
...
async function handleManiaDM(msg) {
    ...
    if (!arg || !guildData) {
        const keys = guilds.map(g => `\`${g.key}\``).join(', ');
        return msg.reply(`❌ Използвай: \`mania-dm <key>\`. Налични: ${keys}`);
    }
