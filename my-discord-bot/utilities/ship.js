...
async function handleMessage(message) {
    ...
    if (cmd === '!ship-captain') {
        ...
        await pool.query(
            `INSERT INTO ship_captains (guild_id, guild_name, user_id, ship_key)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (guild_id, user_id) DO UPDATE SET ship_key = $4, guild_name = $2`,
            [message.guild.id, message.guild.name, targetUser.id, ship.ship_key]
        );
