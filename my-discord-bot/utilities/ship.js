...
async function handleMessage(message) {
    ...
    if (cmd === '!want') {
        ...
        await pool.query(
            `INSERT INTO permanent_crew (guild_id, guild_name, user_id, username, ship_key)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (guild_id, user_id) DO UPDATE SET username = $4, ship_key = $5, guild_name = $2`,
            [message.guild.id, message.guild.name, message.author.id, message.author.username, ship.ship_key]
        );
    ...
    if (cmd === '!ship-captain') {
        ...
        await pool.query(
            `INSERT INTO ship_captains (guild_id, guild_name, user_id, username, ship_key)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (guild_id, user_id) DO UPDATE SET ship_key = $5, guild_name = $2, username = $4`,
            [message.guild.id, message.guild.name, targetUser.id, targetUser.displayName, ship.ship_key]
        );
