...
function initTranslateSystem(client) {
    ...
    client.on('messageCreate', async (message) => {
        ...
        // Skip if text is mostly Latin characters (likely English or typo)
        // Пропускаме ако текстът е предимно латиница
        const latinChars = (cleanText.match(/[a-zA-Z]/g) || []).length;
        const totalChars = cleanText.replace(/\s/g, '').length;
        if (totalChars === 0) return;
        const latinRatio = latinChars / totalChars;
        if (latinRatio > 0.70) return;

        // Skip very short messages (1-2 words) that are likely nicknames/slang
        // Пропускаме много кратки съобщения
        const wordCount = cleanText.split(/\s+/).filter(w => w.length > 0).length;
        if (wordCount <= 2 && cleanText.length < 10) return;
